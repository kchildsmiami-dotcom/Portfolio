...
nurse <- read.csv("data/raw/nurseshark.csv")|>filter(Overall.Probability>0.6)

creef<- read.csv("data/raw/creefshark.csv")|>filter(Overall.Probability>0.6)
...
nurse_raster <- raster(nurse_sf)
creef_raster <- raster(creef_sf)
